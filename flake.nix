{
  ...
  inputs = {
    ...
    comentario = {
      url = "gitlab:comentario/comentario?ref=v3.14.0";
      flake = false;
    };
  };
  outputs =
    ...
    in
    {
      ...
      overlays.default =
        ...
        in
        {
          comentario = final.buildGoModule (finalAttrs: {
            pname = "comentario";
            version = feMeta.version;
            inherit src;
            proxyVendor = true;
            # TODO :: Is there a way to do this without hardcoding this hash?
            vendorHash = "sha256-eHw7kySAYlsii1oZ+OXI98YSnoBNmMQcVhOVj9iReUU=";
            nativeBuildInputs = [
              final.gitMinimal
              final.go-swagger
            ];
          ...
          comentario-fe = final.stdenv.mkDerivation (
            let
      ...
      packages = std.mapAttrs (system: pkgs: {
